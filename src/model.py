...
class UserLogin(db.Model):
    ...
    data = db.relationship('UserData', backref='userlogin')
    swipes = db.relationship('Swipe', backref='userlogin', lazy=True)
...
class UserData(db.Model):
    ...
    user_id = db.Column(db.String(44), db.ForeignKey('userlogin.id'), nullable=False)
...
class Swipe(db.Model):
    ...
    front_user = db.Column(db.String(44), db.ForeignKey('userlogin.id'), nullable=False)
    #The user that is swiped upon
    back_user = db.Column(db.String(44), db.ForeignKey('userlogin.id'), nullable=False)
